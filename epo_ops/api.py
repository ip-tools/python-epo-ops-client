...
from epo_ops.models import AccessToken, Request
...
class Client(object):
    ...
    __classification_cpc_search__ = "classification/cpc/search"
    ...
    def family(self, reference_type, input, endpoint=None, constituents=None):
        ...
        return self._make_request(url, None, params=input.as_api_input(),
                                  use_get=True)
    ...
    def number(self, reference_type, input, output_format):
        ...
        if output_format not in possible_conversions[input_format]:
            raise exceptions.InvalidNumberConversion(
                "Cannot convert from {0} to {1}".format(input_format,
                                                        output_format)
            )
    ...
    def classification_cpc_search(self, search_key):
        return self._cpc_search_request(
            dict(
                service=self.__classification_cpc_search__,
    ...
    def _acquire_token(self):
        ...
        response = requests.post(self.__auth_url__, headers=headers,
                                 data=payload)
    ...
    # info: {service, search_key}
    def _cpc_search_request(self, info):
        url = self._make_request_url(info)
        return self._make_request(
            url, {"q": info.get('search_key', None)}
        )
    ...
    def _image_request(self, path, range, document_format):
        ...
        return self._make_request(
            url, data=data, extra_headers={"Accept": document_format},
            params=params
        )
    ...
    def _check_for_expired_token(self, response):
        ...
        if message.findtext("message") == "invalid_access_token":
            self._acquire_token()
            response = self._make_request(response.request.url,
